...
const url = process.env.CONTINUITY_URL ||
  'http://127.0.0.1:8107/demos/water-scene-lab/waterfall-continuity/?view=ssfr&emitter=highflow';
...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(outputDir, 'surface-ssfr-high-flow-curtain-desktop.png'), fullPage: true });
    ...
    const checks = {
      ...
      highFlowBudget: result.injection.added === 5376 && result.injection.requested === 5376,
      everyTickSchedule: ticks.length === 42 && ticks.every((tick, index) => tick === index),
    ...
    fs.writeFileSync(path.join(outputDir, 'browser-high-flow-results.json'), `${JSON.stringify(report, null, 2)}\n`);
    await browser.close();
  }
