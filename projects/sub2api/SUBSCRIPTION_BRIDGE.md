...
> Sub2API 是位于用户与官方产品后端之间的中间适配转发层。它收到用户消息后，不是把消息交给一个本地 Codex 进程，也不是操作 ChatGPT 网页，而是选择一个已授权的上游账号，将请求转换成 Codex、Claude Code、Gemini CLI 等产品通道所需的网络协议，再用该账号的 OAuth 身份请求官方后端，使请求由供应商按该账号的订阅权限和额度处理。
...
中间适配转发层
  鉴权 → 路由 → 选账号 → 协议转换 → 身份替换
        │ Selected Account OAuth Token
        ▼
Codex / Claude Code / Gemini CLI 对应的官方产品后端
        │ 按上游账号的订阅权益执行
        ▼
响应转换 → 用量记录 → 返回用户
```

其中“走 Sub 逻辑”的准确含义是：官方后端根据 OAuth Token 识别订阅账号，并在供应商侧应用该账号的模型权限、额度窗口和风控规则；Sub2API 自身不会创造、修改或升级这些订阅权益。
