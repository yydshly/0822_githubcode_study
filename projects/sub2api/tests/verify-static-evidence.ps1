...
Assert-FileContains "backend/ent/schema/api_key.go" 'field\.Int64\("group_id"\)' "E16"
Assert-FileContains "backend/ent/schema/account.go" 'Through\("account_groups", AccountGroup\.Type\)' "E16"
Assert-FileContains "backend/internal/service/gateway_scheduling.go" 'ListSchedulableByGroupIDAndPlatform' "E16"
Assert-FileContains "backend/ent/schema/account.go" '"access_token".*"refresh_token".*"expires_at"' "E17"
Assert-FileContains "backend/internal/service/openai_token_provider.go" 'func \(p \*OpenAITokenProvider\) GetAccessToken' "E17"
Assert-FileContains "backend/internal/service/openai_gateway_service.go" 'https://chatgpt\.com/backend-api/codex/responses' "E18"
Assert-FileContains "backend/internal/service/openai_gateway_service.go" 'https://api\.openai\.com/v1/responses' "E18"
Assert-FileContains "backend/internal/service/openai_gateway_forward.go" 'case AccountTypeOAuth:' "E18"
Assert-FileContains "backend/internal/server/middleware/api_key_auth.go" 'apiKeyService\.GetByKey\(c\.Request\.Context\(\), apiKeyString\)' "E19"
Assert-FileContains "backend/internal/service/openai_agent_identity.go" 'headers\.Set\("Authorization", "Bearer "\+token\)' "E19"
Assert-FileContains "backend/internal/service/gateway_upstream_request.go" 'setHeaderRaw\(req\.Header, "authorization", "Bearer "\+token\)' "E19"
