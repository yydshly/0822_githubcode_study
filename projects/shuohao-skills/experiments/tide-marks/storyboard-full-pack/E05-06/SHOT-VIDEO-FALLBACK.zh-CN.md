...
| ---: | --- | --- | ---: | ---: | --- | --- |
| 1 | f1.png | f2.png | 4s | 4s | 可保留精确尾帧，无需裁切 | 许知遥：“六行编号，和货仓清单一模一样。”（逐行核对） |
| 2 | f2.png | f3.png | 3s | 4s | 使用尾帧：整段匀速变速到 3s，禁止尾裁；仅首帧：可在动作完成点尾裁 | 程野指向同一张联单右下角缺一角、向左带短晕尾的暗紫方章：‘右下角，是他的私章。’ |
| 3 | f3.png | f4.png | 4.5s | 5s | 使用尾帧：整段匀速变速到 4.5s，禁止尾裁；仅首帧：可在动作完成点尾裁 | 动作：许知遥用无字回拨控件执行一次倒回操作，随后闭口听录音；画面只显示抽象波形，不显示数字、时间码或按钮文字。 |
| 4 | f4.png | 无 | 3.5s | 4s | 在动作完成点尾裁到 3.5s | 动作：录音深处传来孩子连续的哭声。；许潮：“舱里还有孩子。”（一边跑一边说） |
