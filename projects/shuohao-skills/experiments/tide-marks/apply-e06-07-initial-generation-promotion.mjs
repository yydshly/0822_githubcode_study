...
const segment = "E06-07";
const candidateFolder = path.join("actual-generation", `candidates-${generatedAt}`);
const approvalFile = "offline-production/E06-07-IMAGE-QC-APPROVAL.zh-CN.md";
...
const p06 = "art/images/程野残录手机-sheet.png";
const f1Candidate = "storyboard-full-pack/E06-07/actual-generation/candidates-2026-08-24/f1-v1.png";

const selected = [
  { frame: 1, candidate: "f1-v1.png", references: [pier, cheng, p06, "storyboard-full-pack/E06-06/f2.png"], qc: "通过：只有程野开口；摔裂 P06 与封闭 P03 分离，P06 黑屏仅一条暗青波形，无 UI/时间码" },
  { frame: 2, candidate: "f2-v1.png", references: [pier, gao, f1Candidate], qc: "通过：只有高嵩开口和空手防御手势；P06、P03、其他人物与文字全在画外" },
  { frame: 3, candidate: "f3-v1.png", references: [pier, gao, f1Candidate], qc: "通过：高嵩握自己的完好普通手机开口；恰好两台机械完全静止、灯灭、无排气；无司法结果符号" },
];
...
qc.segments.push({
  id: segment,
  referenceBinding: `${generatedAt} 多参考图条件生成：真实参考顺序、摔裂 P06/封闭 P03、单波形、普通手机区分、机械由怠速转为停机及非司法边界均记录于 actual-generation 文件。`,
  default: {
    consistency: "通过：P06 摔裂且只显示一条暗青波形；P03 继续封闭；高嵩使用自己的完好手机；机械在本段首次依次停机。",
    action: "保留正式 f1-f3；视频阶段严格执行先说后播、P06/P03 状态、手机区分、机械停机过程与非司法结论边界。",
  },
...
console.log(`E06-07 canonical frames promoted: ${selected.length}`);
console.log("E06-07 QC segment updated: 1");
console.log(`E06-07 provenance entries written: ${selected.length}`);
